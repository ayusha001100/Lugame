      ...
      <main className="max-w-6xl mx-auto space-y-12">
        ...
        >
            <div className="mb-10 space-y-2">
              <h1 className="text-7xl font-black tracking-tighter">₹{(kpis.revenue || 0).toLocaleString()}</h1>
              <div className="flex items-center gap-2 text-success font-black italic">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
                { label: 'Revenue', value: `₹${(kpis.revenue || 0).toLocaleString()}`, impact: '+15%', icon: TrendingUp },
                { label: 'ROAS', value: `${(kpis.roas || 0).toFixed(2)}x`, impact: '+0.5', icon: Target },
                { label: 'Conversion', value: `${(kpis.conversionRate || 0).toFixed(1)}%`, impact: '-0.2%', icon: Activity },
                { label: 'CAC', value: `₹${kpis.cac || 0}`, impact: '-120', icon: Users2 },
            ].map((kpi, i) => (
                <motion.div
                    key={kpi.label}
